...
begin
  ...
  -- Scalar-variable RETURNING (no CTE) avoids the editor parser treating `v_result`
  -- as a relation reference under empty search_path.
...
-- 본인 댓글 수정. member_key 일치 안하면 no-op 하고 null 리턴.
...
-- 본인 댓글 삭제. member_key 일치 안하면 no-op 하고 false 리턴.
