/* YouBuddy 비즈니스 영어 챌린지 9기 · Service Worker v1.0 */
const CACHE_NAME = 'youbuddy-9th-v1';

/* ── 설치: 핵심 앱 셸만 캐시 ── */
self.addEventListener('install', (e) => {
  e.waitUntil(
    caches.open(CACHE_NAME).then((cache) =>
      cache.addAll(['/9th/', '/9th/index.html'])
    ).catch(() => {})
  );
  self.skipWaiting();
});
...
self.addEventListener('activate', (e) => {
  e.waitUntil(
    ...
    caches.keys().then((keys) =>
      Promise.all(
        keys.filter((k) => k.startsWith('youbuddy-9th-') && k !== CACHE_NAME)
...
self.addEventListener('fetch', (e) => {
  ...
  e.respondWith(
    fetch(isDoc ? new Request(e.request, { cache: 'no-cache' }) : e.request)
      .then((res) => {
        if (res.ok && e.request.url.includes('/9th/')) {
...
self.addEventListener('push', (e) => {
  let data = { title: 'YouBuddy', body: '새 알림이 있어요', tag: 'default', url: '/9th/' };
...
self.addEventListener('notificationclick', (e) => {
  ...
  const target = (e.notification.data && e.notification.data.url) || '/9th/';
  e.waitUntil(
    clients.matchAll({ type: 'window', includeUncontrolled: true }).then((cs) => {
      const existing = cs.find((c) => c.url.includes('/9th/'));
