...
- [x] ~~**저장한 단어·예문 내보내기**~~ (2026-08-21 완료: 단어장 카드 헤더 📤 내보내기 버튼)
...
- [ ] **9기 앱 세팅: 베이직에 프리미엄 기능 해제** (2026-08-28 상세페이지에 공지 완료, 앱 반영 필수!)
  - AI 첨삭 무제한 (현재 베이직 횟수 제한 로직 해제)
  - 내 문장으로 주간 테스트 보기 (현재 프리미엄 전용 분기 해제)
  - ⚠️ 상세페이지·노션에 이미 약속으로 나가 있으므로 9기 시작 전 반드시 확인.
    "실제로 주지 않을 혜택을 화면에 쓰지 않는다" 원칙: 앱이 안 따라가면 약속 위반이 됨.
