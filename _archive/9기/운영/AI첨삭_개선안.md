...
- Hera (Air cover, 👍, 09-24 02:05 KST): "실제 업무에서 쓰일문장"
  원문 "I need air cover from the director before I push back on this."
  교정 (원문과 동일, verdict correct)
  why (로그에 남은 전체 텍스트): "📌 이건 오늘의 예시문장 그대로예요! 고칠 데가 없는 문장이라 AI 첨삭은 건너뛸게요.\n\n이 문장이 이렇게 쓰인 이유는요, Air cover 를 윗선이 막아주는 지원·방패막이 뜻으로 쓸 때 원어민이 실제로 붙이는 자리와 어순을 그대로 보여주기 때문이에요. (뜻: 이 건에 반대 의견을 내려면 디렉터가 좀 막아줘야 해요.)\n\n한 걸음만 더 가볼까요? 이 문장" ← 여기서 끊김

### 2. 이번 건은 SYSTEM_PROMPT 문제가 아니다

이 케이스는 AI 를 아예 호출하지 않는다. `9th/index.html` 의 `requestSentenceCorrection` 이 제출 문장을 오늘 단어의 예시문장(`word.ex_en`) 또는 유의어 예시문장과 먼저 비교해서(`__matchSampleSentence`), 똑같으면 `supabase/functions/ai-review` 를 부르지 않고 클라이언트에서 만든 고정 문구를 그대로 돌려준다 (9th/index.html:2789~2799). 오늘 Hera 님이 낸 문장이 Day 9 예시문장 원문과 정확히 같아서 이 경로를 탔다. 즉 이 👍 는 SYSTEM_PROMPT 품질과는 무관하고, 고칠 SYSTEM_PROMPT 문구도 없다.

### 3. 대신 찾은 버그: 피드백 로그에 저장될 때 why 가 200자에서 잘림

화면에 보이는 why 는 안 잘린다 (`escapeHtml(fb.why || '')`, 렌더링 쪽엔 slice 없음). 그런데 멤버가 👍/👎 를 보낼 때 로그용으로 저장하는 코드가 잘라버린다.

`9th/index.html:2659`
```js
why: String((fb && fb.why) || '').slice(0, 200),
```

오늘 Hera 님 건의 why 는 약 260자짜리 3단락 문구였는데, 이 slice 때문에 "한 걸음만 더 가볼까요? 이 문장" 에서 끊긴 채로 `AI첨삭_피드백_로그.md` 에 저장됐다. 이 문장 뒤로 원래는 "의 상황을 (본인 role) 쪽으로 바꿔서, 내가 내일 실제로 쓸 문장으로 다시 써보세요. 그때 첨삭이 제일 도움이 됩니다 🧡" 까지 이어진다 (9th/index.html:2798, 하드코딩된 전체 문구 대조 완료).

**멤버는 잘린 걸 못 봤다.** 화면에는 전체 문구가 떴고, 그래서 👍 를 눌렀다. 문제는 순전히 우리 쪽 기록이다: "예시문장 그대로" 케이스처럼 why 가 원래 길게 설계된 응답은 로그만 보고는 뒷부분을 확인할 수 없다.

**09-22 Kate 님 건과 같은 계열일 수 있다.** 그때는 프리미엄 `deep` 필드가 화면에서 끊긴 것처럼 보인다는 제보였고, 원인을 `MAX_TOKENS=700` 추정으로 남겼었다 (09-22 메모). 이번 건으로 확인된 건 최소한 "why" 는 로그 저장 단계에서 잘리는 게 확정이라는 것. `deep` 은 애초에 `state.ai_review_feedback` 에 저장하지 않고 (collect 스크립트도 why 만 읽음), 그래서 Kate 님 건은 다른 경로(실제 OpenAI 응답 끊김 또는 화면 렌더링)일 가능성이 여전히 남아 있다. 둘을 구분하려면 `deep` 도 로그에 같이 저장하도록 바꿔야 한다는 09-22 메모가 다시 유효해진다.

### 4. SYSTEM_PROMPT 수정 제안

이번 건은 해당 없음 (AI 미호출 케이스라 SYSTEM_PROMPT 로 고칠 부분이 없음).

### 5. 제안 외 메모

- 오늘 새 건 1개, 👎 0. SYSTEM_PROMPT 제안 0줄. 대신 `9th/index.html:2659` 의 `slice(0, 200)` 을 늘리거나 (예: 500자) 없애는 걸 권장한다. 프롬프트 파일이 아니라 클라이언트 코드라 여기엔 메모만 남기고, 버디 승인 없이는 건드리지 않았다.
- 같은 파일 2545행에도 같은 모양의 `slice(0, 200)` 이 있는데 이건 `state.ai_history` (재제출 시 에코 판정용 캐시) 저장용이라 별개다. 피드백 로그와는 무관.
- `AI_REVIEW_UPDATES` 공지(9th/index.html:2589)의 `until: '2026-09-24'` 가 오늘로 끝난다. 다음 반영분이 배포되면 새 공지 항목을 추가해야 멤버들이 "내 피드백이 반영됐어요" 를 계속 볼 수 있다.
- 8기는 이번 실행에서 새 피드백 0건 (누적 0건, 아직 챌린지 시작 전이거나 피드백 기능 미사용으로 보임).
