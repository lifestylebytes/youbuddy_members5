...
- **새 버전 배너**: 자동 감지 폐지 (2026-07-15). 멤버에게 "새로고침 하세요" 배너를 띄우려면
  `7th/version.json` 의 `"prompt"` 를 새 문자열로 바꿔 커밋+푸시 (예: `"2026-07-16-w1"`). 비우면 안 뜸.
- **PT 오픈**: `7th/index.html` 의 `__PT_HIDDEN = false` 로.
- **미팅 페이지**: 미팅 당일까지 = "미팅 전 준비" 강조(문장 제출 포함), 다음 날부터 = "복습 시트" 강조.
...
- [ ] Week 3 리텐션 장치 (7/27 전 결정, ③ Day11 오프닝 카드는 완료):
      ① 브리핑에 "2일 연속 미스 케어 큐" + 1:1 초안 자동 생성 ② 홈에 "수료(18일)까지 여유일" 배지
      ④ 월요카드에 지난주 설문 한 줄 인용 (공개 동의자만)
- [ ] 프리미엄 미니 설문 발송 (Week 2 말, 7/23~24). 3문항:
      ① 현재 만족도, 어떤 부분이 가장 만족스러운지
      ② 향후 개선되었으면 하는 점
      ③ 유비챌에 지속 참여한다면 어떤 점이 가장 크게 긍정적으로 작용했기 때문일지
      (재등록 80% 목표의 중간 측정. 결과는 W3~4 운영과 수료식 재등록 오퍼에 반영)
